...
while cur_err > err_max  % sum(abs(err))
    ...
    switch code_switch
        case 1  % 原代码
            if cur_err < delta_switch
                joint_passive = joint_passive + alpha * pinv(J_all'*J_all + tol2 * eye(24)) * J_all' * err;
            else
                joint_passive = joint_passive + alpha * ((J_all'*J_all + tol*eye(24)) \ (J_all' * err));
            end
            ...
            if(loop < loop_max)
                loop = loop + 1;
                cur_err = norm(err);
                err_list(loop) = cur_err;  % sum(abs(err))
            else
                break;
            end

        case 2  % 变阻尼
            % pk = alpha * ((J_all'*J_all + mu*eye(24)) \ (J_all' * err));
            pk = ((J_all'*J_all + mu*eye(24)) \ (J_all' * err));
            joint_passive = joint_passive + pk;
            % disp("rank J_all'*J_all");
            % disp(rank(J_all'*J_all));
            ...
            err = err_cal(T0);
            last_err = cur_err;
            cur_err = norm(err);  % 每次用于循环的err

            delta_err = cur_err - last_err;
            delta_qk = -1 *( (J_all'*err)'*pk + 0.5*pk'*(J_all'*J_all)*pk );
        ...
        otherwise  % AI写的
            ...
            delta_q = alpha * (J_all'*J_all + mu * eye(24)) \ (J_all' * err);
